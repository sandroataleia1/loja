...
function computeSummary(movements: CashMovement[], openingBalance: number) {
  const supplies    = movements.filter((m) => m.type === 'supply')
                               .reduce((s, m) => s + m.amount_cents, 0)
  const withdrawals = movements.filter((m) => m.type === 'withdrawal')
                               .reduce((s, m) => s + m.amount_cents, 0)
  // Sales movements represent cash inflows recorded by the backend
  const salesCash   = movements.filter((m) => m.type === 'sale')
                               .reduce((s, m) => s + m.amount_cents, 0)
...
export default function FecharCaixaPage() {
  ...
  const { supplies, withdrawals, salesCash, expected } = computeSummary(
    movements,
