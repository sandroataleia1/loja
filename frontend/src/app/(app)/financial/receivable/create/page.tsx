  ...
  return (
    <div className="space-y-6">
      <AppPageHeader
        ...
        description="Registre uma nova receita ou valor a receber."
        actions={
          <Button variant="outline" asChild>
            <Link href={ROUTES.FINANCIAL_RECEIVABLE}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        <AppCard title="Dados do Lançamento">
          <div className="grid gap-4">
            <div className="grid gap-1.5">
              ...
              <Input id="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex: Venda, Mensalidade, Serviço..." />
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="grid gap-1.5">
                ...
                >
                  ...
                  {incomeCategories.map((c) => <option key={c.uuid} value={c.uuid}>{c.name}</option>)}
                </select>
              </div>
              <div className="grid gap-1.5">
              ...
              <div className="space-y-3">
                {installments.map((inst, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <Input type="date" value={inst.due_date} onChange={(e) => updateInstallment(i, 'due_date', e.target.value)} className="w-40" />
                    <Input value={inst.amount} onChange={(e) => updateInstallment(i, 'amount', e.target.value)} placeholder="0,00" className="w-32" />
        ...
        <div className="flex gap-3">
          ...
          <Button type="button" variant="outline" asChild>
            <Link href={ROUTES.FINANCIAL_RECEIVABLE}>Cancelar</Link>
          </Button>
        </div>
      </form>
    </div>
