...
export function PdvKeybar({ onSearchOrder }: Props) {
  return (
    <footer className="flex items-center gap-1 px-3 py-1.5 border-t bg-muted/30 text-xs text-muted-foreground shrink-0 select-none">
      {BASE_KEYS.map(([key, label]) => (
        <span
          key={key}
          className={`flex items-center gap-1 mr-3 ${key === 'F3' && onSearchOrder ? 'cursor-pointer hover:text-foreground' : ''}`}
          onClick={key === 'F3' ? onSearchOrder : undefined}
        >
